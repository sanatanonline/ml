...
class Perceptron:
    ...
    # And now we can use the Perceptron. We generate two dataset, the first one is used to train the classifier (
    # train set), and the second one is used to test it (test set):
    @staticmethod
    def main():
        trainset = Perceptron.generateData(30)
        perceptron = Perceptron()
        perceptron.train(trainset)
        testset = Perceptron.generateData(20)
        for x in testset:
